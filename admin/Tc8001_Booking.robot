...
Open Browser And Admin
    [Documentation]  ทดสอบการเลื่อนเวลา
...
Booking
    ...
    ${Reschedule_Tiger_Locator}=    Set Variable    //tr[.//td[text()='Tiger']]//button[text()='เลื่อนเวลา']
    Wait Until Element Is Visible    ${Reschedule_Tiger_Locator}    timeout=10s
    Click Element    ${Reschedule_Tiger_Locator}
    
    # 2. เลือกเวลาใหม่
    Click Element    xpath://button[text()='07:00']
    
    # 3. ยืนยันการเลื่อนเวลา
    Click Element    xpath://button[text()='ยืนยันการเลื่อนเวลา']
    
    # 4. ตรวจสอบข้อความยืนยันผลลัพธ์
    Wait Until Page Contains    อัปเดตการจองสำเร็จ!
